        ...
        self.observation_space = spaces.Dict(
            dict(
                ...
                joints_state=spaces.Box(
                    -np.inf, np.inf, shape=obs["joints_state"].shape, dtype="float64"
                ),
                realsense_rgb_image=spaces.Box(
                    0, 255, shape=obs["realsense_rgb_image"].shape, dtype="uint8"
                ),
                realsense_depth_image=spaces.Box(
                    -np.inf, np.inf, shape=obs["realsense_depth_image"].shape, dtype="float32"
                ),
                # realsense_pointcloud=spaces.Box(
                #     -np.inf, np.inf, shape=obs["realsense_pointcloud"].shape, dtype="float64"
                # ),
                realsense2_rgb_image=spaces.Box(
                    0, 255, shape=obs["realsense2_rgb_image"].shape, dtype="uint8"
                ),
                realsense2_depth_image=spaces.Box(
                    -np.inf, np.inf, shape=obs["realsense2_depth_image"].shape, dtype="float32"
                ),
                # kinect_pointcloud=spaces.Box(
                #     -np.inf, np.inf, shape=obs["kinect_pointcloud"].shape, dtype="float64"
                # ),
            )
        )
...
class MujocoRobotEnv(BaseRobotEnv):
    ...
    def render(self):
        ...
        # rgb_image = self.mujoco_renderer.render("rgb_array", camera_name="realsense")
        # depth_image = self.mujoco_renderer.render("depth_array", camera_name="realsense")
        # rgb_image_kinect = self.mujoco_renderer.render("rgb_array", camera_name="kinect")
        # depth_image_kinect = self.mujoco_renderer.render("depth_array", camera_name="kinect")
